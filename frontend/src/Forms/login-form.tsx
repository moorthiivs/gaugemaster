...
function CustomGoogleButton({ onToken, loading }: { onToken: (token: string) => Promise<void>; loading: boolean }) {
    ...
    return (
        <Button
            ...
            onClick={() => login()}
            disabled={isBusy}
            className="w-full flex items-center justify-center gap-3 h-10 text-sm font-medium border border-border hover:bg-muted transition-all shadow-sm rounded-md"
        >
            ...
            <span>{isBusy ? "Signing in with Google..." : "Continue with Google"}</span>
        </Button>
    );
}
...
export function LoginForm() {
    ...
    // Check if Google authentication is enabled on the server and client ID is available
                    ...
                    <>
                        ...
                        {authConfig?.googleClientId ? (
                            <GoogleOAuthProvider clientId={authConfig.googleClientId}>
                                <CustomGoogleButton onToken={handleGoogleToken} loading={loading} />
                            </GoogleOAuthProvider>
                        ) : (
                            <CustomGoogleButton onToken={handleGoogleToken} loading={loading} />
                        )}
